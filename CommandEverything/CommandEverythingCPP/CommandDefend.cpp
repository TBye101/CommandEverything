...
void CommandDefend::DefendStart()
{
	...
	for (register unsigned __int8 i = 0; i < drives; i++)
	{
		...
		drive = strbuffer[0];
		drive.append("://");
		Console->WriteLine(&("Locking files on drive " + drive));
		this->LockFiles(Utility->ToCharStar(&drive), 0);
		Console->WriteLine("Done locking files on drive");
	}
}
...
void CommandDefend::LockFiles(char* name, int indent = 0)
{
	register unsigned __int64 totalFiles = Utility->GraphicCalculateFilesIn(name, indent);
	...
	while ((entry = readdir(dir)) != NULL)
	{
		...
		if (entry->d_type == DT_DIR)
		{
			...
			this->LockFiles(path, indent + 2);
		}
		else
		{
			//Do stuff per each file
			char* convert = entry->d_name;
			wchar_t* wtext;
			mbstowcs(wtext, convert, strlen(convert) + 1);//Plus null
			LPWSTR ptr = wtext;

			CreateFile(ptr, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		}
	}
	closedir(dir);
}
...
#include "cusparse_v2.h"  /* CUSPARSE public header file */
...
int CUSPARSE_SET_STREAM(ptr_t *handle, ptr_t *streamId){
    return (int)cusparseSetStream((cusparseHandle_t)(*handle), (cudaStream_t)(*streamId)); 
}

int CUSPARSE_GET_POINTER_MODE(ptr_t *handle, int *mode){
    cusparsePointerMode_t tmode = 0;
    int error = (int)cusparseGetPointerMode((cusparseHandle_t)(*handle), &tmode);
    *mode = (int)tmode;
    return error;
}

int CUSPARSE_SET_POINTER_MODE(ptr_t *handle, int *mode){
    return (int)cusparseSetPointerMode((cusparseHandle_t)(*handle), (cusparsePointerMode_t)(*mode));
}
...
int CUSPARSE_SHYBMV(ptr_t *handle,
                    int *transA, 
                    const float *alpha,
                    const ptr_t *descrA, 
                    const ptr_t *hybA, 
                    ...
                    const float *beta, 
                    ptr_t *y){
    return (int)cusparseShybmv((cusparseHandle_t)(*handle),
                               (cusparseOperation_t)(*transA),
                               alpha,
                               (const cusparseMatDescr_t)(*descrA),
...
int CUSPARSE_DHYBMV(ptr_t *handle,
                    int *transA, 
                    const double *alpha,
                    const ptr_t *descrA, 
                    const ptr_t *hybA, 
                    ...
                    const double *beta, 
                    ptr_t *y){
    return (int)cusparseDhybmv((cusparseHandle_t)(*handle),
                               (cusparseOperation_t)(*transA),
                               alpha,
                               (const cusparseMatDescr_t)(*descrA),
                               (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_CHYBMV(ptr_t *handle,
                    int *transA, 
                    const cuComplex *alpha,
                    const ptr_t *descrA, 
                    const ptr_t *hybA, 
                    ...
                    const cuComplex *beta, 
                    ptr_t *y){
    return (int)cusparseChybmv((cusparseHandle_t)(*handle),
                               (cusparseOperation_t)(*transA),
                               alpha,
                               (const cusparseMatDescr_t)(*descrA),
                               (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_ZHYBMV(ptr_t *handle,
                    int *transA, 
                    const cuDoubleComplex *alpha,
                    const ptr_t *descrA, 
                    const ptr_t *hybA, 
                    ...
                    const cuDoubleComplex *beta, 
                    ptr_t *y){
    return (int)cusparseZhybmv((cusparseHandle_t)(*handle),
                               (cusparseOperation_t)(*transA),
                               alpha,
                               (const cusparseMatDescr_t)(*descrA),
                               (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_SHYBSV_ANALYSIS(ptr_t *handle, 
                             int *transA, 
                             const ptr_t *descrA, 
                             const ptr_t *hybA, 
                             ptr_t *info){
    return (int)cusparseShybsv_analysis((cusparseHandle_t)(*handle), 
                                        (cusparseOperation_t)(*transA), 
                                        (const cusparseMatDescr_t)(*descrA), 
...
int CUSPARSE_DHYBSV_ANALYSIS(ptr_t *handle, 
                             int *transA, 
                             const ptr_t *descrA, 
                             const ptr_t *hybA, 
                             ptr_t *info){
    return (int)cusparseDhybsv_analysis((cusparseHandle_t)(*handle), 
                                        (cusparseOperation_t)(*transA), 
                                        (const cusparseMatDescr_t)(*descrA), 
                                        (cusparseHybMat_t)(*hybA),
                                        (cusparseSolveAnalysisInfo_t)(*info));
...
int CUSPARSE_CHYBSV_ANALYSIS(ptr_t *handle, 
                             int *transA, 
                             const ptr_t *descrA, 
                             const ptr_t *hybA, 
                             ptr_t *info){
    return (int)cusparseChybsv_analysis((cusparseHandle_t)(*handle), 
                                        (cusparseOperation_t)(*transA), 
                                        (const cusparseMatDescr_t)(*descrA), 
                                        (cusparseHybMat_t)(*hybA),
                                        (cusparseSolveAnalysisInfo_t)(*info));
...
int CUSPARSE_ZHYBSV_ANALYSIS(ptr_t *handle, 
                             int *transA, 
                             const ptr_t *descrA, 
                             const ptr_t *hybA, 
                             ptr_t *info){
    return (int)cusparseZhybsv_analysis((cusparseHandle_t)(*handle), 
                                        (cusparseOperation_t)(*transA), 
                                        (const cusparseMatDescr_t)(*descrA), 
                                        (cusparseHybMat_t)(*hybA),
                                        (cusparseSolveAnalysisInfo_t)(*info));
...
int CUSPARSE_SHYBSV_SOLVE(ptr_t *handle, 
                          int *transA, 
                          const float *alpha, 
                          const ptr_t *descrA, 
                          const ptr_t *hybA, 
                          ...
                          const ptr_t *x, 
                          ptr_t *y){
    return (int)cusparseShybsv_solve((cusparseHandle_t)(*handle), 
                                     (cusparseOperation_t)(*transA), 
                                     alpha, 
                                     (const cusparseMatDescr_t)(*descrA),
                                     (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_DHYBSV_SOLVE(ptr_t *handle, 
                          int *transA, 
                          const double *alpha, 
                          const ptr_t *descrA, 
                          const ptr_t *hybA, 
                          ...
                          const ptr_t *x, 
                          ptr_t *y){
    return (int)cusparseDhybsv_solve((cusparseHandle_t)(*handle), 
                                     (cusparseOperation_t)(*transA), 
                                     alpha, 
                                     (const cusparseMatDescr_t)(*descrA),
                                     (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_CHYBSV_SOLVE(ptr_t *handle, 
                          int *transA, 
                          const cuComplex *alpha, 
                          const ptr_t *descrA, 
                          const ptr_t *hybA, 
                          ...
                          const ptr_t *x, 
                          ptr_t *y){
    return (int)cusparseChybsv_solve((cusparseHandle_t)(*handle), 
                                     (cusparseOperation_t)(*transA), 
                                     alpha, 
                                     (const cusparseMatDescr_t)(*descrA),
                                     (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_ZHYBSV_SOLVE(ptr_t *handle, 
                          int *transA, 
                          const cuDoubleComplex *alpha, 
                          const ptr_t *descrA, 
                          const ptr_t *hybA, 
                          ...
                          const ptr_t *x, 
                          ptr_t *y){
    return (int)cusparseZhybsv_solve((cusparseHandle_t)(*handle), 
                                     (cusparseOperation_t)(*transA), 
                                     alpha, 
                                     (const cusparseMatDescr_t)(*descrA),
                                     (const cusparseHybMat_t)(*hybA),
...
int CUSPARSE_SDENSE2HYB(ptr_t *handle,
                        int *m, 
                        int *n,  
                        const ptr_t *descrA,                            
                        const ptr_t *A, 
                        ...
                        int *userEllWidth,
                        int *partitionType){
    return (int)cusparseSdense2hyb((cusparseHandle_t)(*handle),
                                   *m,
                                   *n,
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const float *)(*A),
...
int CUSPARSE_DDENSE2HYB(ptr_t *handle,
                        int *m, 
                        int *n,  
                        const ptr_t *descrA,                            
                        const ptr_t *A, 
                        ...
                        int *userEllWidth,
                        int *partitionType){
    return (int)cusparseDdense2hyb((cusparseHandle_t)(*handle),
                                   *m,
                                   *n,
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const double *)(*A),
...
int CUSPARSE_CDENSE2HYB(ptr_t *handle,
                        int *m, 
                        int *n,  
                        const ptr_t *descrA,                            
                        const ptr_t *A, 
                        ...
                        int *userEllWidth,
                        int *partitionType){
    return (int)cusparseCdense2hyb((cusparseHandle_t)(*handle),
                                   *m,
                                   *n,
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cuComplex *)(*A),
...
int CUSPARSE_ZDENSE2HYB(ptr_t *handle,
                        int *m, 
                        int *n,  
                        const ptr_t *descrA,                            
                        const ptr_t *A, 
                        ...
                        int *userEllWidth,
                        int *partitionType){
    return (int)cusparseZdense2hyb((cusparseHandle_t)(*handle),
                                   *m,
                                   *n,
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cuDoubleComplex *)(*A),
...
int CUSPARSE_SHYB2DENSE(ptr_t *handle,
                        const ptr_t *descrA,                            
                        ptr_t *hybA, 
                        const ptr_t *A, 
                        int *lda){
    return (int)cusparseShyb2dense((cusparseHandle_t)(*handle),
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cusparseHybMat_t)(*hybA),
                                   (float *)(*A),
                                   *lda);
...
int CUSPARSE_DHYB2DENSE(ptr_t *handle,
                        const ptr_t *descrA,                            
                        ptr_t *hybA, 
                        const ptr_t *A, 
                        int *lda){
    return (int)cusparseDhyb2dense((cusparseHandle_t)(*handle),
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cusparseHybMat_t)(*hybA),
                                   (double *)(*A),
                                   *lda);
...
int CUSPARSE_CHYB2DENSE(ptr_t *handle,
                        const ptr_t *descrA,                            
                        ptr_t *hybA, 
                        const ptr_t *A, 
                        int *lda){
    return (int)cusparseChyb2dense((cusparseHandle_t)(*handle),
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cusparseHybMat_t)(*hybA),
                                   (cuComplex *)(*A),
                                   *lda);
...
int CUSPARSE_ZHYB2DENSE(ptr_t *handle,
                        const ptr_t *descrA,                            
                        ptr_t *hybA, 
                        const ptr_t *A, 
                        int *lda){
    return (int)cusparseZhyb2dense((cusparseHandle_t)(*handle),
                                   (const cusparseMatDescr_t)(*descrA),
                                   (const cusparseHybMat_t)(*hybA),
                                   (cuDoubleComplex *)(*A),
                                   *lda);
...
int CUSPARSE_SCSR2HYB(ptr_t *handle,
                      int *m, 
                      int *n, 
                      const ptr_t *descrA,
                      const ptr_t *csrValA, 
                      ...
                      int *userEllWidth,
                      int *partitionType){
    return (int)cusparseScsr2hyb((cusparseHandle_t)(*handle),
                                 *m,
                                 *n,
                                 (const cusparseMatDescr_t)(*descrA),
                                 (const float *)(*csrValA),
...
int CUSPARSE_DCSR2HYB(ptr_t *handle,
                      int *m, 
                      int *n, 
                      const ptr_t *descrA,
                      const ptr_t *csrValA, 
                      ...
                      int *userEllWidth,
                      int *partitionType){
    return (int)cusparseDcsr2hyb((cusparseHandle_t)(*handle),
                                 *m,
                                 *n,
                                 (const cusparseMatDescr_t)(*descrA),
                                 (const double *)(*csrValA),
...
int CUSPARSE_CCSR2HYB(ptr_t *handle,
                      int *m, 
                      int *n, 
                      const ptr_t *descrA,
                      const ptr_t *csrValA, 
                      ...
                      int *userEllWidth,
                      int *partitionType){
    return (int)cusparseCcsr2hyb((cusparseHandle_t)(*handle),
                                 *m,
                                 *n,
                                 (const cusparseMatDescr_t)(*descrA),
                                 (const cuComplex *)(*csrValA),
...
int CUSPARSE_ZCSR2HYB(ptr_t *handle,
                      int *m, 
                      int *n, 
                      const ptr_t *descrA,
                      const ptr_t *csrValA, 
                      ...
                      int *userEllWidth,
                      int *partitionType){
    return (int)cusparseZcsr2hyb((cusparseHandle_t)(*handle),
                                 *m,
                                 *n,
                                 (const cusparseMatDescr_t)(*descrA),
                                 (const cuDoubleComplex *)(*csrValA),
